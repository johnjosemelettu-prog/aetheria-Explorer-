...
import { type RecommendWardrobeInput, type RecommendWardrobeOutput, type RecommendedItem } from '@/ai/flows/wardrobe-schemas'
...
import { generateWardrobeAction, analyzeBodyScanAction } from './actions'
...
const looks = [
  { id: 'look-1', title: 'Positano Linen', image: PlaceHolderImages.find((img) => img.id === 'digital-tailor-look-1') },
  { id: 'look-2', title: 'Tokyo Streetwear', image: PlaceHolderImages.find((img) => img.id === 'digital-tailor-look-2') },
  { id: 'look-3', title: 'SF Tech Fleece', image: PlaceHolderImages.find((img) => img.id === 'digital-tailor-look-3') },
]
...
export default function DigitalTailorPage() {
  ...
  const [wardrobeRequest, setWardrobeRequest] = useState<RecommendWardrobeInput>({
    fitPreference: 'regular',
    sensitivities: [],
    styleVibe: { lovedLooks: [], passedLooks: [] },
    brandAffinities: [],
    tripContext: {
  ...
  useEffect(() => {
    if (currentStep === 0 && activeTab === 'synthesize') {
      const getCameraPermission = async () => {
        try {
          if (hasCameraPermission === null && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
  ...
  const captureAndAnalyze = async () => {
    ...
    try {
      ...
      if (ctx) {
        ...
        const result = await analyzeBodyScanAction(base64Image);
        
        if (result?.fitPreference) {
          onUpdate({ fitPreference: result.fitPreference });
          toast({
            title: "Scan Complete",
            description: `Detected Preference: ${result.fitPreference.toUpperCase()}.\nFeatures: ${result.detectedFeatures?.join(', ')}`,
          });
        }
      }
    } catch (error) {
      ...
      toast({
        variant: "destructive",
        title: "Scan Failed",
        description: "Could not complete body analysis."
      });
    } finally {
      setIsScanning(false);
    }
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-12 md:grid-cols-2">
      ...
      <div className="space-y-4 flex flex-col items-center">
        <div className="relative flex aspect-[3/4] w-full max-w-md items-center justify-center overflow-hidden rounded-[2rem] border-4 border-slate-100 bg-slate-900 shadow-2xl">
          <video
            ref={videoRef}
        ...
        {hasCameraPermission === true && (
          <Button 
            ...
            disabled={isScanning}
            className="rounded-full h-12 px-8 font-black uppercase tracking-widest shadow-xl bg-primary text-white w-full max-w-md"
          >
            {isScanning ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Camera className="mr-2 h-4 w-4" />}
            {isScanning ? 'Analyzing Morphology...' : 'Capture & Analyze Body Type'}
          </Button>
        )}
  ...
  useEffect(() => {
    if (!recommendations && !error) {
        async function getRecommendations() {
            setIsLoading(true)
            try {
                const result = await generateWardrobeAction(requestData)
