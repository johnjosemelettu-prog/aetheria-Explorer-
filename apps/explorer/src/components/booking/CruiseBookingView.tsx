    ...
    return (
        <Card className="mx-auto max-w-2xl border-none shadow-2xl rounded-[2rem] md:rounded-[2.5rem] overflow-hidden animate-in zoom-in duration-500">
          <div className="bg-indigo-600 p-6 md:p-10 text-white text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 p-6 md:p-10 opacity-10"><Ship className="h-24 w-24 md:h-40 md:w-40" /></div>
            <div className="relative z-10">
              ...
              <CardTitle className="text-2xl sm:text-3xl md:text-4xl font-black font-headline tracking-tighter">Ocean Bound!</CardTitle>
              <CardDescription className="text-white/80 mt-2 text-base md:text-lg font-medium">You are booked on the {selectedCruise.name}.</CardDescription>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 animate-in slide-in-from-bottom-4 duration-500">
          <div className="lg:col-span-7 space-y-6 md:space-y-8">
            <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2.5rem] bg-white overflow-hidden">
              <CardHeader className="bg-indigo-950 text-white p-6 md:p-8">
                <CardTitle className="text-xl md:text-2xl font-black font-headline flex items-center gap-3">
                  <Fingerprint className="h-5 w-5 md:h-6 md:w-6 text-primary" /> Voyage Manifest
                </CardTitle>
                <CardDescription className="text-slate-400 text-xs md:text-sm">Complete traveler details for maritime boarding.</CardDescription>
              </CardHeader>
            ...
            <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2.5rem] bg-white p-6 md:p-8">
              <h3 className="font-headline font-black text-lg md:text-xl mb-4 md:mb-6 text-slate-900 flex items-center gap-2">
                <Wallet className="h-5 w-5 md:h-6 md:w-6 text-indigo-600" /> Payment Node
              </h3>
              <div className="space-y-3 md:space-y-4">
                ...
                <div className="flex justify-between items-center p-3 md:p-4 rounded-xl md:rounded-2xl bg-indigo-50 border border-indigo-100">
                  <span className="text-[10px] font-black uppercase text-indigo-600">Grand Total Voyage</span>
                  <span className="font-headline font-black text-lg md:text-xl text-indigo-600">${totalPrice.toFixed(2)}</span>
                </div>
              </div>
            </Card>
          </div>
          ...
          <aside className="lg:col-span-5">
            <Card className="border-none shadow-2xl rounded-[2rem] md:rounded-[3rem] bg-slate-900 text-white overflow-hidden sticky top-24">
              <div className="relative h-40 md:h-48 w-full">
                ...
                <div className="absolute bottom-4 left-6">
                  <CardTitle className="text-2xl md:text-3xl font-black font-headline leading-tight">{selectedCruise.name}</CardTitle>
                  <CardDescription className="text-indigo-400 font-bold uppercase tracking-widest text-[9px] md:text-[10px]">{selectedCruise.cruiseLine}</CardDescription>
                </div>
              </div>
              <CardContent className="p-6 md:p-8 space-y-4 md:space-y-6">
                <div className="grid grid-cols-2 gap-4 md:gap-6 py-4 md:py-6 border-y border-white/10">
                  <div>
                    <p className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Duration</p>
                    <p className="font-bold text-sm md:text-base">{selectedCruise.durationDays} Days</p>
                  </div>
                  <div>
                    <p className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Departure</p>
                    <p className="font-bold text-sm md:text-base">{selectedCruise.destination}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <p className="text-[10px] md:text-xs text-slate-400 font-medium leading-relaxed">Secure protocol active. Your booking is protected by Aura Journey Assurance.</p>
                </div>
              </CardContent>
              <CardFooter className="p-6 md:p-8 pt-0 gap-3">
                <Button variant="ghost" className="flex-1 font-bold text-white/40 hover:text-white h-10 md:h-12" onClick={() => setStep('search')}>Back</Button>
                <Button
                    className="flex-[2] h-12 md:h-14 rounded-xl md:rounded-2xl font-black text-base md:text-lg bg-indigo-600 hover:bg-indigo-700 shadow-xl"
      ...
      <div className="space-y-6 md:space-y-10">
        <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden">
          <CardContent className="p-6 md:p-10">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 items-end gap-4 md:gap-6 md:grid-cols-3">
                <FormField control={form.control} name="destination" render={({ field }) => (
                    <FormItem><FormLabel className="font-bold text-xs md:text-sm">{t('cruises.destinationLabel')}</FormLabel><FormControl><Input placeholder={t('cruises.destinationPlaceholder')} {...field} className="rounded-lg md:rounded-xl h-10 md:h-12 text-xs md:text-sm" /></FormControl></FormItem>
                )} />
                <FormField control={form.control} name="departureMonth" render={({ field }) => (
                    <FormItem><FormLabel className="font-bold text-xs md:text-sm">{t('cruises.monthLabel')}</FormLabel><FormControl><Input type="month" {...field} className="rounded-lg md:rounded-xl h-10 md:h-12 text-xs md:text-sm" /></FormControl></FormItem>
                )} />
                <Button type="submit" className="h-12 md:h-14 rounded-xl md:rounded-2xl font-black text-base md:text-lg shadow-xl shadow-indigo-200 bg-indigo-500 hover:bg-indigo-600 text-white" disabled={isLoading}>
        ...
        {searchResults && (
            <div className="grid grid-cols-1 gap-6 md:gap-10 md:grid-cols-2 lg:grid-cols-3">
              {searchResults.map((cruise) => (
                  <Card key={cruise.id} className="overflow-hidden border-none shadow-xl rounded-[1.5rem] md:rounded-[2rem] group flex flex-col bg-white">
                    {cruise.image && (
                    ...
                    <CardHeader className="p-6 md:p-8 pb-3 md:pb-4"><CardTitle className="text-xl md:text-2xl font-black font-headline text-slate-900 leading-tight">{cruise.name}</CardTitle><CardDescription className="flex items-center gap-2 font-bold text-indigo-500 uppercase tracking-widest text-[9px] md:text-[10px]"><Sailboat className="h-3 w-3 md:h-3.5 md:w-3.5"/> {cruise.cruiseLine}</CardDescription></CardHeader>
                    <CardContent className="px-6 md:px-8 pb-3 md:pb-4 flex-grow"><p className="text-xs md:text-sm font-medium text-slate-500 mb-3 md:mb-4">{cruise.durationDays} days in {cruise.destination}</p><div className="flex items-baseline gap-1"><span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('cruises.from')}</span><span className="text-2xl md:text-3xl font-black font-headline text-slate-900">${cruise.pricePerPerson}</span><span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">/ PERSON</span></div></CardContent>
                    <CardFooter className="p-6 md:p-8 pt-0"><Button className="w-full h-10 md:h-12 rounded-lg md:rounded-xl font-black bg-slate-900 text-white hover:bg-indigo-600 text-xs md:text-sm" onClick={() => handleSelectCruise(cruise)}>Select Cabin</Button></CardFooter>
                  </Card>
              ))}
            </div>
        )}
