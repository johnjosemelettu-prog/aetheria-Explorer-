    ...
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      <div className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-muted-foreground font-medium">
          Turn your city exploration into a high-stakes scavenger hunt.
        </p>
      </div>
      ...
      {!questData ? (
        <Card className="max-w-md mx-auto border-none shadow-2xl rounded-[2.5rem] overflow-hidden">
          <CardHeader className="bg-slate-900 text-white p-8">
            <CardTitle className="text-xl uppercase tracking-tighter italic">Initialize Quest Grid</CardTitle>
            <CardDescription className="text-slate-400">Select a target city to synthesize custom exploration missions.</CardDescription>
          </CardHeader>
          ...
          <CardFooter className="p-8 pt-0">
            <Button onClick={handleGenerate} className="w-full h-14 rounded-2xl font-black text-lg shadow-xl shadow-primary/20" disabled={isLoading}>
              {isLoading ? <Loader2 className="animate-spin mr-2" /> : <><Sparkles className="mr-2 h-5 w-5" /> Generate Quest</>}
            </Button>
          </CardFooter>
        </Card>
      ) : (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <Card className="bg-primary text-primary-foreground border-none shadow-xl rounded-[2.5rem] overflow-hidden">
            ...
            <CardContent className="p-8 pt-0 space-y-4">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-1">
                <span>Progress: {completedIds.length} / {questData.quests.length}</span>
                <span className="text-accent">{currentPoints} / {totalPoints} XP</span>
              </div>
                ...
                >
                  <CardContent className="p-6 flex items-start gap-6">
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <p className="text-sm text-slate-500 font-medium leading-relaxed italic mb-3">Hint: {quest.hint}</p>
                      <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-primary">
                        <Sparkles className="h-3.5 w-3.5" />
                        {quest.points} XP Synthesized
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {completedIds.length === questData.quests.length && (
            <div className="text-center py-16 bg-emerald-50 rounded-[3rem] border-2 border-dashed border-emerald-200 animate-in zoom-in duration-700">
              ...
              <h2 className="text-4xl font-black font-headline text-emerald-900 uppercase italic tracking-tighter">City Resolved!</h2>
              <p className="text-emerald-700 font-medium mt-2">You've successfully decoded the secrets of {city}.</p>
              <Button asChild className="mt-8 rounded-2xl font-black h-14 px-10 bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-200">
                <Link href="/dashboard">Return to Command Hub <Sparkles className="ml-2 h-4 w-4" /></Link>
              </Button>
            </div>
          )}
        </div>
